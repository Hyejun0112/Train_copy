...
# 리듀서 사이즈 표기: 베이스 도면 텍스트는 "80X","150X"처럼 '숫자+x'로 끝나고
# (뒤에 숫자가 없음), 그 바로 뒤에 사용자가 빨간 숫자 마크업("50")을 겹쳐 그려
# 합쳐서 "80X50"처럼 읽히게 한다 — 즉 베이스 텍스트와 마크업은 서로 다른 객체.
# 도면이 틀어져도 마크업("50")이 베이스 텍스트("80X") 바로 뒤를 따라가야 한다
# (허공에 뜨면 안 됨). Train copy마다 실제 배관 사이즈 값 자체는 다를 수 있으므로
# (소스 "8X" ↔ 타깃 "8X"여도 마크업 값 "50"은 위치 기준일 뿐), 텍스트 일치가
# 아니라 '위치상 가장 가까운 베이스 텍스트'로 대응시킨다.
_REDUCER_SIZE_RE = re.compile(r'^\d{1,4}\s*[xX×]$')  # "80X","150X" 등(뒤에 숫자 없음)
